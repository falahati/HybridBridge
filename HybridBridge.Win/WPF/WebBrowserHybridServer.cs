...
namespace HybridBridge.Win.WPF
{
    /// <summary>
    ///     WebBrowserHybridServer is a class containing a list of registered
    ///     <see cref="IBridgeHandler" /> instances for WPF WebBrowser control.
    /// </summary>
    public class WebBrowserHybridServer : HybridBridgeLocalServer
    {
        ...
        private async void WebViewOnNavigated(object sender, NavigationEventArgs navigationEventArgs)
        {
        ...
        {
            if (result == null)
                InvokeOnUiThread(() => { WebView.InvokeScript(@"eval", jsCode); });
            else
                Task.Factory.StartNew(() =>
                {
                    string value = null;
                    InvokeOnUiThread(
                        () => { value = WebView.InvokeScript(@"eval", jsCode) as string; });
